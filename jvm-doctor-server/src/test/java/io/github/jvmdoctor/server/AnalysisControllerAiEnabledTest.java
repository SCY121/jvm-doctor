...
class AnalysisControllerAiEnabledTest {
    ...
    static void registerProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("jvm-doctor.ai.base-url", AnalysisControllerAiEnabledTest::baseUrl);
    ...
    @Test
    void shouldReturnAiAugmentationWhenEnabled() throws Exception {
        mockMvc.perform(multipart("/api/v1/analyses")
                        .file(threadDump())
                        .file(actuatorMetrics())
                        .file(logFile()))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.ai.status").value("COMPLETED"))
                .andExpect(jsonPath("$.ai.provider").value("openai-compatible"))
                .andExpect(jsonPath("$.ai.model").value("test-model"))
                .andExpect(jsonPath("$.ai.summary").value("Database access is the leading bottleneck in this incident."))
                .andExpect(jsonPath("$.ai.recommendedActions[0]").value("Inspect slow SQL and connection hold time first."))
                .andExpect(jsonPath("$.ai.missingEvidence[0]").value("JFR snapshot"));
    }
    ...
    private static synchronized String baseUrl() {
        if (httpServer == null) {
            ...
            httpServer.createContext("/chat/completions", exchange -> write(exchange, 200, """
                    {
                      "id": "chatcmpl-test",
                      "choices": [
                        {
                          "message": {
                            "content": "{\\"summary\\":\\"Database access is the leading bottleneck in this incident.\\",\\"recommendedActions\\":[\\"Inspect slow SQL and connection hold time first.\\",\\"Capture one more thread dump during peak load.\\"],\\"missingEvidence\\":[\\"JFR snapshot\\",\\"Database slow query log\\"],\\"riskNote\\":\\"Use only the deterministic evidence as the source of truth.\\"}"
                          }
                        }
                      ]
                    }
